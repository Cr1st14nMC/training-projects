<template>
  <div>
    <!-- Alerts -->
    <div v-if="alert.show" :class="['alert', alert.type === 'success' ? 'alert-success' : 'alert-danger', 'alert-dismissible', 'fade', 'show']" role="alert">
      {{ alert.message }}
    ...
    <table class="table table-bordered align-middle">
      ...
      <tbody>
        <tr v-for="producto in productos" :key="producto.id">
          ...
          <td>
            <button @click="goEdit(producto.id)" class="btn btn-warning btn-sm me-2">
              <i class="bi bi-pencil-square me-1"></i>Editar
            </button>
            <button @click="confirmDelete(producto.id)" class="btn btn-danger btn-sm">
  ...
  methods: {
    async fetchProductos() {
      ...
      try {
        ...
        // asumiendo la estructura { success: true, data: [...] }
        this.productos = (res.data && res.data.data) ? res.data.data : [];
      } catch (err) {
        console.error(err);
    ...
    async confirmDelete(id) {
      ...
      try {
        ...
        if (res.data && res.data.success) {
          ...
          this.showAlert(res.data.message || 'No se pudo eliminar', 'error');
        }
      } catch (err) {
        console.error(err);
